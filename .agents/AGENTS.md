...
2. **KHÔNG TỰ THÊM DEPENDENCY MỚI (NO UNAPPROVED DEPENDENCIES):**
   - Chỉ được sử dụng các thư viện đã được phê duyệt trong [architecture.md](file:///e:/Code/nodetask/docs/architecture.md).
...
3. **TUÂN THỦ QUY TẮC UI KHÔNG ICON (ZERO-ICON RULE):**
   - Kiểm tra kỹ skill [.agents/skills/minimalist-no-icon-ui/SKILL.md](file:///e:/Code/nodetask/.agents/skills/minimalist-no-icon-ui/SKILL.md).
...
4. **KHÔNG TỰ Ý REFACTOR API HOẶC DB SCHEMA CŨ:**
   - Khi sửa code, luôn tuân theo Endpoints & Schema trong [data_and_api.md](file:///e:/Code/nodetask/docs/data_and_api.md).
